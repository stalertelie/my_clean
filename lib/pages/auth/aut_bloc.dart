...
class AuthBloc extends BaseBloc {
  ...
  createUser(User c){
    ...
    response.then((value) {
      ...
      print(error);
      loadingSubject.add(Loading(message: error.toString().removeExeptionWord(),hasError: true,loading: false));
      GetIt.I<AppServices>().showSnackbarWithState(loadingSubject.value);
    });
  }
  ...
  logUser(User c){
    ...
    Future<dynamic> response =  requestExtension.post("authentication_token", jsonEncode(c));
  ...
  getUserByToken(String token){
    ...
    Future<dynamic> response =  requestExtension.get(UrlConstant.url_users+"/${payload['id']}");

    response.then((value) {
      User user = value as User;
      UtilsFonction.saveData(AppConstant.USER_INFO, json.encode(user)).then((value) {
        //_loadingSubject.add(Loading(loading: false,hasError: false,message: "Connecion ok"),);
        loadingSubject.add(Loading(message: MessageConstant.loginok,hasError: false, loading: false,data: user));
      });
    }).catchError((error){
      print(error);
      loadingSubject.add(Loading(message: error.toString().removeExeptionWord(),hasError: true,loading: false));
